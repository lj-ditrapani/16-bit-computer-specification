...
Up to 3 Data ROM banks can be added to the cartridge.  They cannot contain CPU instructions, only data.  Each Data ROM bank is a pair of matching 8-wide ROM chips like PGR ROM.
The cartridge receives 3 Data ROM device enable lines from the CPU, to allow bank switching between 3 Data ROM banks.
The max number of ROM chips on a cartridge is 8: 2 for Program (PGR) ROM, and up to 6 for Data ROM.  A fully populated cartridge with 64K x 8 chips is 512 KB.
